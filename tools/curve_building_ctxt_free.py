    ...
    for i in range(nbFullWindows):
        for word in wordfreq.tokenize(sliding_windows[i], 'en'):
            ...
            if word in gloveDic.keys():
                if scale:
                    embeddings[i] += gloveDic[word] * np.exp(5 * np.log(1- word_frequency(word, 'en')))
                else:
                    embeddings[i] += gloveDic[word]
            elif meanVec:
                nonVocGlove += 1
                if scale and (word_frequency(word, 'en') != 0):
                    embeddings[i] += meanVec * np.exp(5 * np.log(1- word_frequency(word, 'en')))
                else:
